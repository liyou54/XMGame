...
namespace XM.ConfigNew.CodeGen
{
    ...
    public static class TypeHelper
    {
        ...
        /// <summary>
        /// 判断是否继承自 TypeConverterBase（有静态 I 单例，应使用 .I.Convert() 而非 new T().Convert()）
        /// </summary>
        public static bool IsTypeConverterBase(Type type)
        {
            if (type == null)
                return false;
            var typeConverterBaseDef = typeof(TypeConverterBase<object, object, object>).GetGenericTypeDefinition();
            var t = type;
            while (t != null && t != typeof(object))
            {
                var cur = t.IsGenericType ? t.GetGenericTypeDefinition() : t;
                if (cur == typeConverterBaseDef)
                    return true;
                t = t.BaseType;
            }
            return false;
        ...
        /// <summary>
        /// XMLLink 字段不再使用后缀
        /// Link 字段直接存储父节点的 CfgI
        /// </summary>
        public static string LinkParentSuffix => "";
        
        /// <summary>
        /// 生成 XMLLink 父节点索引名称
        /// </summary>
        /// <param name="parentTypeName">父节点类型名</param>
        /// <returns>索引名称，格式：ByParent_{ParentTypeName}</returns>
        public static string GenerateParentLinkIndexName(string parentTypeName)
        {
            return CodeGenConstants.ParentLinkIndexPrefix + parentTypeName;
        }
        ...
        public static string GetUnmanagedElementTypeName(Type type)
        {
            ...
            {
                // 通过程序集 XmlTypeConverter 推导（如 XAssetPath -> AssetI）
                var unmanagedFromConverter = Tools.TypeAnalyzer.GetUnmanagedTypeFromXmlTypeConverter(type);
                if (unmanagedFromConverter != null)
                    return GetGlobalQualifiedTypeName(unmanagedFromConverter);
            }
        ...
        public static string GetUnmanagedFieldTypeName(ConfigFieldMetadata field, Func<EXmlStrMode, string> getStringModeTypeName = null)
        {
            ...
            // 6.5. 通过 XmlTypeConverter 分析得到的托管->非托管转换（优先于硬编码）
            if (field.Converter?.UnmanagedTargetType != null)
            {
                return GetGlobalQualifiedTypeName(field.Converter.UnmanagedTargetType);
            }
            
            // 6.6. 从程序集 XmlTypeConverter 推导（用于无字段元数据场景，如容器元素）
            var unmanagedFromConverter = Tools.TypeAnalyzer.GetUnmanagedTypeFromXmlTypeConverter(targetType);
            if (unmanagedFromConverter != null)
            {
                return GetGlobalQualifiedTypeName(unmanagedFromConverter);
            }
            ...
            if (ns != null)
            {
                ...
                if (ns.StartsWith("XM"))
                {
                    ...
                    if (typeName == "LabelS" || typeName == "ModS" || typeName == "TblS")
                        return false;
                    // 有 XmlTypeConverter 转换为其他类型的，说明本身非 unmanaged
                    if (Tools.TypeAnalyzer.GetUnmanagedTypeFromXmlTypeConverter(type) != null)
                        return false;
