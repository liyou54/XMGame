...
namespace XM.ConfigNew.CodeGen
{
    ...
    public class XmlHelperGenerator
    {
        ...
        private void GenerateStaticConstructor()
        {
            ...
            _builder.AppendLine($"const string __modName = \"{modName}\";");
            _builder.AppendLine($"CfgS<{unmanagedTypeName}>.Table = new TblS(new ModS(__modName), __tableName);");
            _builder.AppendLine("TblS = new TblS(new ModS(__modName), __tableName);");
            // 在静态构造函数中创建实例
        ...
        private void GenerateInterfaceMethods()
        {
            ...
            // GetLinkHelperType
            GenerateGetLinkHelperTypeMethod();
        }
        ...
        /// <summary>
        /// 生成 GetLinkHelperType 方法
        /// </summary>
        private void GenerateGetLinkHelperTypeMethod()
        {
            _builder.BeginMethod("override Type GetLinkHelperType()", "获取 Link Helper 类型");
            
            // TODO: 根据 Link 关系返回正确的类型
            _builder.AppendLine("return null;");
        ...
        #region EstablishLinks 方法
        
        /// <summary>
        /// 生成 EstablishLinks 方法
        /// </summary>
        private void GenerateEstablishLinksMethod()
        {
            ...
            _builder.AppendLine("public virtual void EstablishLinks(");
            _builder.PushIndent();
            _builder.AppendLine($"{managedTypeName} config,");
            _builder.AppendLine($"ref {unmanagedTypeName} data,");
            _builder.AppendLine("XM.ConfigDataCenter.ConfigDataHolder configHolderData)");
            _builder.PopIndent();
            _builder.BeginBlock();
            
            _builder.AppendComment("TODO: 实现 Link 双向引用");
            _builder.AppendComment("父→子: 通过 CfgI 查找子配置，填充 XBlobPtr");
            _builder.AppendComment("子→父: 通过 CfgI 查找父配置，填充引用");
        ...
        private string GetXmlLinkUnmanagedTypeName(Type targetManagedType)
        {
            ...
            var ns = targetManagedType.Namespace;
            
            if (!string.IsNullOrEmpty(ns))
            {
                return $"global::{ns}.{unmanagedTypeName}";
            }
