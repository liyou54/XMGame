...
{
    /// <summary>
    /// ActiveSkillConfig 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
    /// </summary>
    public class ActiveSkillConfigClassHelper : ConfigClassHelper<global::XM.ConfigNew.Tests.Data.ActiveSkillConfig, global::XM.ConfigNew.Tests.Data.ActiveSkillConfigUnmanaged>
    {
        public static ActiveSkillConfigClassHelper Instance { get; private set; }
        ...
        /// 静态构造函数
        /// </summary>
        static ActiveSkillConfigClassHelper()
        {
            const string __tableName = "ActiveSkillConfig";
            const string __modName = "Default";
            CfgS<global::XM.ConfigNew.Tests.Data.ActiveSkillConfigUnmanaged>.Table = new TblS(new ModS(__modName), __tableName);
            TblS = new TblS(new ModS(__modName), __tableName);
            Instance = new ActiveSkillConfigClassHelper();
        }
        /// <summary>
        /// 构造函数
        /// </summary>
        public ActiveSkillConfigClassHelper()
        {
        }
        ...
        {
            var config = (global::XM.ConfigNew.Tests.Data.ActiveSkillConfig)target;

            // 解析所有字段
            config.ManaCost = ParseManaCost(configItem, mod, configName, context);
            config.Cooldown = ParseCooldown(configItem, mod, configName, context);
            config.SkillEffects = ParseSkillEffects(configItem, mod, configName, context);
        ...
        /// 解析 ManaCost 字段
        /// </summary>
        private static int ParseManaCost(
        ...
        {
            var xmlValue = global::XM.Contracts.Config.ConfigParseHelper.GetXmlFieldValue(configItem, "ManaCost");
            ...
            if (global::XM.Contracts.Config.ConfigParseHelper.TryParseInt(xmlValue, "ManaCost", out var parsedValue))
            {
                return parsedValue;
            }
        ...
        /// 解析 Cooldown 字段
        /// </summary>
        private static float ParseCooldown(
            global::System.Xml.XmlElement configItem,
            global::XM.Contracts.Config.ModS mod,
            string configName,
            in global::XM.Contracts.Config.ConfigParseContext context)
        {
            var xmlValue = global::XM.Contracts.Config.ConfigParseHelper.GetXmlFieldValue(configItem, "Cooldown");
            ...
            if (global::XM.Contracts.Config.ConfigParseHelper.TryParseFloat(xmlValue, "Cooldown", out var parsedValue))
            {
                return parsedValue;
            }
        ...
        /// 解析 SkillEffects 字段
        /// </summary>
        private static global::System.Collections.Generic.List<global::XM.ConfigNew.Tests.Data.EffectConfig> ParseSkillEffects(
            global::System.Xml.XmlElement configItem,
            global::XM.Contracts.Config.ModS mod,
            string configName,
            in global::XM.Contracts.Config.ConfigParseContext context)
        {
            ...
            // 尝试从 XML 节点解析
            var nodes = configItem.SelectNodes("SkillEffects");
            if (nodes != null)
            {
            ...
            if (list.Count == 0)
            {
                var csvValue = global::XM.Contracts.Config.ConfigParseHelper.GetXmlFieldValue(configItem, "SkillEffects");
        ...
        public override void AllocContainerWithFillImpl(
            ...
            TblI tbli,
            CfgI cfgi,
            ref global::XM.ConfigNew.Tests.Data.ActiveSkillConfigUnmanaged data,
            XM.ConfigDataCenter.ConfigDataHolder configHolderData,
            XBlobPtr? linkParent = null)
        {
            var config = (global::XM.ConfigNew.Tests.Data.ActiveSkillConfig)value;

            // 分配容器和嵌套配置
            AllocSkillEffects(config, ref data, cfgi, configHolderData);
        ...
        public virtual void EstablishLinks(
            global::XM.ConfigNew.Tests.Data.ActiveSkillConfig config,
            ref global::XM.ConfigNew.Tests.Data.ActiveSkillConfigUnmanaged data,
            XM.ConfigDataCenter.ConfigDataHolder configHolderData)
        {
            // TODO: 实现 Link 双向引用
        ...
        /// 分配 SkillEffects 容器
        /// </summary>
        private void AllocSkillEffects(global::XM.ConfigNew.Tests.Data.ActiveSkillConfig config, ref global::XM.ConfigNew.Tests.Data.ActiveSkillConfigUnmanaged data, CfgI cfgi, XM.ConfigDataCenter.ConfigDataHolder configHolderData)
        {
            if (config.SkillEffects == null || config.SkillEffects.Count == 0)
            {
                return;
            }

            var array = configHolderData.Data.BlobContainer.AllocArray<global::XM.ConfigNew.Tests.Data.EffectConfigUnmanaged>(config.SkillEffects.Count);
            var helper = global::XM.ConfigNew.Tests.Data.EffectConfigClassHelper.Instance;
            if (helper != null)
            {
                for (int i = 0; i < config.SkillEffects.Count; i++)
                {
                    if (config.SkillEffects[i] != null)
                    {
                        var itemData = new global::XM.ConfigNew.Tests.Data.EffectConfigUnmanaged();
                        helper.AllocContainerWithFillImpl(config.SkillEffects[i], default(TblI), cfgi, ref itemData, configHolderData);
                        array[configHolderData.Data.BlobContainer, i] = itemData;
                    }
                }
            }

            data.SkillEffects = array;
        }
