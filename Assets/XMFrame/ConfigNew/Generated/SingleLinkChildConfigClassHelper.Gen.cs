...
{
    /// <summary>
    /// SingleLinkChildConfig 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
    /// </summary>
    public class SingleLinkChildConfigClassHelper : ConfigClassHelper<global::XM.ConfigNew.Tests.Data.SingleLinkChildConfig, global::XM.ConfigNew.Tests.Data.SingleLinkChildConfigUnmanaged>
    {
        public static SingleLinkChildConfigClassHelper Instance { get; private set; }
        ...
        /// 静态构造函数
        /// </summary>
        static SingleLinkChildConfigClassHelper()
        {
            const string __tableName = "SingleLinkChild";
            const string __modName = "Default";
            CfgS<global::XM.ConfigNew.Tests.Data.SingleLinkChildConfigUnmanaged>.Table = new TblS(new ModS(__modName), __tableName);
            TblS = new TblS(new ModS(__modName), __tableName);
            Instance = new SingleLinkChildConfigClassHelper();
        }
        /// <summary>
        /// 构造函数
        /// </summary>
        public SingleLinkChildConfigClassHelper()
        {
        }
        ...
        {
            var config = (global::XM.ConfigNew.Tests.Data.SingleLinkChildConfig)target;
        ...
        /// 解析 Parent 字段
        /// </summary>
        private static global::XM.Contracts.Config.CfgS<global::XM.ConfigNew.Tests.Data.LinkParentConfig> ParseParent(
            global::System.Xml.XmlElement configItem,
            global::XM.Contracts.Config.ModS mod,
            string configName,
            in global::XM.Contracts.Config.ConfigParseContext context)
        {
            // 解析 CfgS 引用字符串
            var cfgSString = global::XM.Contracts.Config.ConfigParseHelper.GetXmlFieldValue(configItem, "Parent");
            if (string.IsNullOrEmpty(cfgSString))
            {
                return default;
            }

            if (global::XM.Contracts.Config.ConfigParseHelper.TryParseCfgSString(cfgSString, "Parent", out var modName, out var cfgName))
            {
                return new global::XM.Contracts.Config.CfgS<global::XM.ConfigNew.Tests.Data.LinkParentConfig>(new global::XM.Contracts.Config.ModS(modName), cfgName);
            }

            global::XM.Contracts.Config.ConfigParseHelper.LogParseError(context, "Parent", $"无法解析 CfgS 字符串: {cfgSString}");
            return default;
        }
        ...
        public override void AllocContainerWithFillImpl(
            ...
            TblI tbli,
            CfgI cfgi,
            ref global::XM.ConfigNew.Tests.Data.SingleLinkChildConfigUnmanaged data,
            XM.ConfigDataCenter.ConfigDataHolder configHolderData,
            XBlobPtr? linkParent = null)
        {
            var config = (global::XM.ConfigNew.Tests.Data.SingleLinkChildConfig)value;

            // 填充基本类型字段
            data.ChildId = config.ChildId;
            data.ChildName = new global::Unity.Collections.FixedString32Bytes(config.ChildName ?? string.Empty);
            // TODO: Parent - CfgS 转 CfgI (链接阶段解析)
            if (TryGetCfgI(config.Parent, out var ParentCfgI))
            {
                data.Parent = ParentCfgI.As<global::XM.ConfigNew.Tests.Data.LinkParentConfigUnmanaged>();
            }
        }
        /// <summary>
        /// 建立 Link 双向引用（链接阶段调用）
        ...
        public virtual void EstablishLinks(
            global::XM.ConfigNew.Tests.Data.SingleLinkChildConfig config,
            ref global::XM.ConfigNew.Tests.Data.SingleLinkChildConfigUnmanaged data,
            XM.ConfigDataCenter.ConfigDataHolder configHolderData)
        {
            // TODO: 实现 Link 双向引用
