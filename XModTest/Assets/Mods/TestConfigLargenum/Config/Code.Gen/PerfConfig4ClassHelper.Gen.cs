...
/// <summary>
/// PerfConfig4 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig4ClassHelper : ConfigClassHelper<PerfConfig4, PerfConfig4UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig4ClassHelper()
    {
        const string __tableName = "PerfConfig4";
        CfgS<PerfConfig4UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig4UnManaged));
    ...
    public PerfConfig4ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig4UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig4UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
