...
/// <summary>
/// PerfConfig9 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig9ClassHelper : ConfigClassHelper<PerfConfig9, PerfConfig9UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig9ClassHelper()
    {
        const string __tableName = "PerfConfig9";
        CfgS<PerfConfig9UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig9UnManaged));
    ...
    public PerfConfig9ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig9UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig9UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
