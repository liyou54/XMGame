...
/// <summary>
/// PerfConfig5 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig5ClassHelper : ConfigClassHelper<PerfConfig5, PerfConfig5UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig5ClassHelper()
    {
        const string __tableName = "PerfConfig5";
        CfgS<PerfConfig5UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig5UnManaged));
    ...
    public PerfConfig5ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig5UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig5UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
