...
/// <summary>
/// PerfConfig3 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig3ClassHelper : ConfigClassHelper<PerfConfig3, PerfConfig3UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig3ClassHelper()
    {
        const string __tableName = "PerfConfig3";
        CfgS<PerfConfig3UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig3UnManaged));
    ...
    public PerfConfig3ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig3UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig3UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
