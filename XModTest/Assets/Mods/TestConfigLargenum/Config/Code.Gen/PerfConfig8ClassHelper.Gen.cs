...
/// <summary>
/// PerfConfig8 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig8ClassHelper : ConfigClassHelper<PerfConfig8, PerfConfig8UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig8ClassHelper()
    {
        const string __tableName = "PerfConfig8";
        CfgS<PerfConfig8UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig8UnManaged));
    ...
    public PerfConfig8ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig8UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig8UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
