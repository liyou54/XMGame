...
/// <summary>
/// PerfConfig6 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig6ClassHelper : ConfigClassHelper<PerfConfig6, PerfConfig6UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig6ClassHelper()
    {
        const string __tableName = "PerfConfig6";
        CfgS<PerfConfig6UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig6UnManaged));
    ...
    public PerfConfig6ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig6UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig6UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
