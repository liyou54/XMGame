...
/// <summary>
/// PerfConfig1 的配置加载辅助类，用于从 XML 反序列化（静态代码生成，无反射）。
/// </summary>
public sealed class PerfConfig1ClassHelper : ConfigClassHelper<PerfConfig1, PerfConfig1UnManaged>
{
    public static TblI TblI { get; private set; }
    public static TblS TblS { get; private set; }

    static PerfConfig1ClassHelper()
    {
        const string __tableName = "PerfConfig1";
        CfgS<PerfConfig1UnManaged>.TableName = __tableName;
        try
        {
            var __cfgSType = typeof(CfgS<>).MakeGenericType(typeof(PerfConfig1UnManaged));
    ...
    public PerfConfig1ClassHelper(IConfigDataCenter dataCenter)
        : base(dataCenter)
    {
    }
    ...
    private static CfgS<PerfConfig1UnManaged> ParseId(XmlElement configItem, ModS mod, string configName)
        {
            try
            {
            ...
            return new CfgS<PerfConfig1UnManaged>(new ModS(modName), cfgName);
            }
            catch (Exception ex)
            {
